...
// Generado programaticamente desde el array POLYGLOT_RANDOM_ARRAY de python-chess
// (github.com/niklasf/python-chess, chess/polyglot.py), que a su vez documenta
// venir del codigo fuente original de PolyGlot de Fabien Letouzey.
// Verificado: 781 valores, primer y ultimo valor coinciden con multiples fuentes
// independientes (chessprogramming.org, hgm.nubati.net, TalkChess).
pub const POLYGLOT_RANDOM64: [u64; 781] = [
...
pub fn polyglot_hash(board: &Board) -> u64 {
    ...
    if board.castling.white_kingside { h ^= r[768]; }
    if board.castling.white_queenside { h ^= r[769]; }
    if board.castling.black_kingside { h ^= r[770]; }
    if board.castling.black_queenside { h ^= r[771]; }
    ...
    if let Some(ep) = board.en_passant {
        ...
        let pawn_rank = if us == Color::White { ep_rank - 1 } else { ep_rank + 1 };
...
fn decode_polyglot_move(raw: u16, legal: &[Move]) -> Option<Move> {
    ...
    legal.iter().find(|mv| mv.from == from_sq && mv.to == to_sq && mv.promotion() == promotion).copied()
}
...
mod tests {
    ...
    fn startpos_book_moves_match_python_chess() {
        ...
        let as_strings: Vec<(String, u32)> = results.iter().map(|(m, w)| (m.to_string(), *w)).collect();
        assert_eq!(
            as_strings,
            vec![
                ("e2e4".to_string(), 50),
    ...
    fn castling_encoding_quirk_is_translated_correctly() {
        ...
        let book = Book::load(TEST_BOOK).unwrap();
        let board =
            Board::from_fen("r1bqk2r/pppp1ppp/2n2n2/2b1p3/2B1P3/2N2N2/PPPP1PPP/R1BQK2R w KQkq - 6 5").unwrap();
        let results = book.probe(&board);
        assert_eq!(results.len(), 1);
    ...
    fn en_passant_position_book_move() {
        let book = Book::load(TEST_BOOK).unwrap();
        let board =
            Board::from_fen("rnbqkbnr/ppp1p1pp/8/3pPp2/8/8/PPPP1PPP/RNBQKBNR w KQkq f6 0 4").unwrap();
        let results = book.probe(&board);
        assert_eq!(results.len(), 1);
